...
#define GPSPOWER_LAT LATAbits.LATA7
#define GPSPOWER_PORT IOPORT_A,BIT_7


#define GPSEN_TRIS TRISAbits.TRISA10
#define GPSEN_LAT LATAbits.LATA10
#define GPSEN_PORT IOPORT_A,BIT_10
...
void GPS_Init(void) {
    ...
    UARTSetFifoMode(UART2, UART_INTERRUPT_ON_RX_NOT_EMPTY | UART_INTERRUPT_ON_TX_BUFFER_EMPTY);
    ...
    GPSPOWER_TRIS = 0;
    PORTSetPinsDigitalOut(GPSPOWER_PORT);
    GPSEN_TRIS = 0;
...
void __ISR(_UART2_VECTOR, ipl4) IntUart2Handler(void) {
    ...
    if (INTGetFlag(INT_U2TX)) {
        if (!(getLength(GtransmitBuffer) == 0)) {
            transmitcount++;
            U2TXREG = readFront(GtransmitBuffer);
